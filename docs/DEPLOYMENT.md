...
| **P3009** (fehlgeschlagene Migration, API-Restart-Schleife) | Erst **`rolled-back`** wie in **[SUPABASE.md](./SUPABASE.md)** („P3009 / init — Schleife“): Wenn danach **erneut P3009** mit **neuem Zeitstempel** → Init läuft wieder an und scheitert (z. B. Tabellen schon da) — dann **`scripts/supabase-wipe-public-schema.sql`** (nur wenn DB wegwerfbar) oder gezielt **`--applied`**. Kurz: nicht **`rolled-back`** in einer Endlosschleife. |
